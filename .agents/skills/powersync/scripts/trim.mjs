...
const MATCHERS = [
  ...
  {
    platform: "swift",
    match: (n) => n === "Package.swift" || n === "Podfile",
    pattern: /powersync/i,
  },
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--apply") args.apply = true;
      ...
      console.error(
        "Usage: node scripts/trim.mjs [--apply | --decline] [--repo <path>] [--keep <p1,p2>] [--json]",
      );
      process.exit(2);
    }
  }
  ...
  if (invalid.length > 0) {
    console.error(
      `Unknown --keep platform(s): ${invalid.join(", ")} (valid: ${Object.keys(PLATFORMS).join(", ")})`,
    );
    process.exit(2);
  }
  return args;
...
function classifyInstall(repoArg) {
  ...
  if (root) {
    if (existsSync(join(root, ".claude-plugin", "marketplace.json"))) {
      ...
      if (rel === join("skills", "powersync") || rel.startsWith("skills" + sep)) {
        return {
          kind: "source",
          detail: "this is the skill source repository; trim only installed copies",
        };
      }
    }
    if (!isInside(root, SKILL_DIR)) {
      return {
        kind: "unknown",
        detail: "the skill directory is not inside the repo root; pass --repo <path>",
      };
    }
  ...
  if (isInside(homedir(), SKILL_DIR)) {
    return {
      kind: "shared",
      detail: "this looks like a user-level install shared across projects",
    };
  }
...
function detectPlatforms(root) {
  ...
  const walk = (dir, depth) => {
    ...
    for (const entry of entries) {
      ...
      let lines;
      try {
        lines = readFileSync(full, "utf-8").split(/\r?\n/);
      } catch {
        continue;
      }
...
function findDanglingRefs(removedPaths, prospective) {
  ...
  for (const file of collectMdFiles(SKILL_DIR)) {
    ...
    const content =
      prospective && prospective[rel] !== undefined
        ? prospective[rel]
...
function applyTrim(keptPlatforms, removedPaths, say) {
  ...
  if (missing.length > 0) {
    ...
    console.error(
      "This copy looks partially modified. Reinstall or run `npx skills update`, then retry.",
    );
    process.exit(1);
  }
  if (
    !prospective["SKILL.md"] ||
    !prospective["SKILL.md"].startsWith("---") ||
    !prospective["SKILL.md"].includes("name: powersync")
  ) {
    console.error(
      "Rewrite verification failed: SKILL.md frontmatter would be damaged. Nothing was written.",
    );
    process.exit(1);
  }
...
function main() {
  ...
  if (existsSync(statePath)) {
    ...
    if (args.apply) {
      say(
        `A decision is already recorded in ${STATE_FILE} (${decision}). Delete that file to decide again.`,
      );
      process.exit(2);
    }
    say(
      `Decision already recorded in ${STATE_FILE}: ${decision} (${state && state.date ? state.date : "unknown date"}). Nothing to do.`,
    );
    process.exit(0);
  }
  ...
  if (detectedList.length === 0) {
    emit({
      install: { kind: install.kind, root: install.root },
      detected: {},
      wouldRemove: [],
      applied: false,
    });
  ...
  const kept = Object.keys(PLATFORMS).filter(
    (p) => detectedList.includes(p) || args.keep.includes(p),
  );
  ...
  if (!args.json) {
    ...
    for (const p of Object.keys(PLATFORMS)) {
      ...
      say(
        `${p.padEnd(10)} ${flag.padEnd(9)} ${ev ? `${ev.file}:${ev.line} ${ev.text}` : ""}`.trimEnd(),
      );
    }
    say("");
  }
